...
const STATUS_LABEL: Record<string,string> = { open:"受付", investigating:"調査中", waiting:"対応待ち", in_progress:"対応中", done:"完了", unknown:"不明" };
...
export default function AdminDashboard(){
  ...
  const userChartData = useMemo(()=> (users?.rows??[]).map(r=>({ owner:r.owner, 合計:r.total, 受付:r.open, 調査中:r.investigating, 対応待ち:r.waiting, 対応中:r.in_progress, 完了:r.done })), [users]);
  ...
  return (<div className="space-y-6">
    ...
    <div className="flex flex-wrap items-center gap-3">
      ...
      {mode==="day" ? (
        <div className="inline-flex rounded-xl border overflow-hidden">
          {[7,14,30,90].map(v=> <button key={v} className={`px-3 py-1.5 text-sm ${days===v ? "bg-[#E6F2FB] text-[#0f5ea8]" : "bg-white hover:bg-gray-50"}`} onClick={()=>setDays(v)}>{v}日</button>)}
        </div>
      ) : (
        <div className="inline-flex rounded-xl border overflow-hidden">
          {[3,6,12].map(v=> <button key={v} className={`px-3 py-1.5 text-sm ${months===v ? "bg-[#E6F2FB] text-[#0f5ea8]" : "bg-white hover:bg-gray-50"}`} onClick={()=>setMonths(v)}>{v}ヶ月</button>)}
        </div>
      )}
    </div>
    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
      <KpiCard label="総チケット数" value={stats?.total??0} />
      <KpiCard label="本日作成" value={stats?.today??0} />
      <KpiCard label={mode==="day" ? `${days}日合計` : `${months}ヶ月合計`} value={seriesTotal} />
    </div>
    ...
    {users && users.rows.length>0 && (<>
      <div className="flex items-center justify-between">
        ...
        <button onClick={()=>{ const q=mode==="day"? `/api/tickets/stats/users?granularity=day&days=${days}&format=csv` : `/api/tickets/stats/users?granularity=month&months=${months}&format=csv`; const a=document.createElement("a"); a.href=q; a.download=""; document.body.appendChild(a); a.click(); a.remove(); }} className="px-3 py-1.5 text-sm rounded-xl border hover:bg-[#E6F2FB]">CSVダウンロード</button>
      </div>
      <div className="bg-white border rounded-2xl p-4 shadow-sm">
        <div className="font-semibold mb-2">ユーザー別 件数（合計）</div>
        <div style={{width:"100%",height:360}}>
          <ResponsiveContainer>
            <BarChart layout="vertical" data={userChartData.slice(0,15)}><CartesianGrid strokeDasharray="3 3"/><XAxis type="number" allowDecimals={false}/><YAxis type="category" dataKey="owner" width={160}/><Tooltip/><Legend/><Bar dataKey="合計" fill={BRAND.primary}/></BarChart>
          </ResponsiveContainer>
        </div>
      </div>
      <div className="bg-white border rounded-2xl p-4 shadow-sm">
        ...
        <div style={{width:"100%",height:420}}>
          <ResponsiveContainer>
            <BarChart layout="vertical" data={userChartData.slice(0,15)}><CartesianGrid strokeDasharray="3 3"/><XAxis type="number" allowDecimals={false}/><YAxis type="category" dataKey="owner" width={160}/><Tooltip/><Legend/><Bar dataKey="受付" stackId="a" fill={BRAND.gray}/><Bar dataKey="調査中" stackId="a" fill={BRAND.orange}/><Bar dataKey="対応待ち" stackId="a" fill={BRAND.yellow}/><Bar dataKey="対応中" stackId="a" fill={BRAND.primary}/><Bar dataKey="完了" stackId="a" fill={BRAND.green}/></BarChart>
          </ResponsiveContainer>
        </div>
      </div>
      <div className="overflow-auto rounded-xl border bg-white">
        <table className="min-w-[880px] w-full text-sm"><thead className="bg-gray-50 text-gray-600"><tr><th className="text-left px-4 py-2">ユーザー</th><th className="text-right px-4 py-2">合計</th><th className="text-right px-4 py-2">受付</th><th className="text-right px-4 py-2">調査中</th><th className="text-right px-4 py-2">対応待ち</th><th className="text-right px-4 py-2">対応中</th><th className="text-right px-4 py-2">完了</th></tr></thead>
